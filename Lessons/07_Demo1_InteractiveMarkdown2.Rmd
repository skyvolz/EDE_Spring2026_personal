...
selectInput(
  inputId = "nutrient", 
  label = "Select a Nutrient",  #what appears in webpage
  choices = c('tn_ug', 'tp_ug', 'nh34'),   #choices in dropdown
  selected = 'tn-ug'     #default seleted value
) 
...
sliderInput(
  ...
  step = 1,
  sep = ''     #get rid of comma in the years
)

# Create a renderPlot object to display the plot 
# created with the plot_it function
renderPlot(plot_it(
  input$nutrient,     #from selector widget inputId
  input$years[1],
  input$years[2]      #from slider inputId
))
