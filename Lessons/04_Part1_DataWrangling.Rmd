...
> Add notes here: Temperature, oxygen, and light all determine plant growth in lakes and support the base of the food chain for aquatic animals.
...
`!=` #shows if variables are different
`<` #less than
`<=` #less than or equal to
`>` #greater than
`>=`#greater than or equal to 
`&` #want both to be true
`|` #want one or the other to be true - OR operator
...
# dplyr filtering - first argument is data set, second is filter
...
NTL.phys.data.90s <- filter(NTL.phys.data, year4 > 1989, year4 < 2000, lakename == "Tuesday Lake")
```
Question: Why don't we filter using row numbers?

> Answer: There are multiple values for each row
...
slice(NTL.phys.data.temperature.descending, 1:3)
...
#we keep wrangling code separate because we can see how we have filtered out data. Also, if the analysis gets messed up it won't mess up your wrangling code. This keeps the wrangled code seperate like you would for a raw dataset. Keeping them separate also makes cleaner R scripts.
