...
> Add notes here: Can look at long term patterns in temperate / oxygen / light as climate conditions change. This data determines plant growth which forms the basis of the food chain and ecosystem health.
...
levels(NTL.phys.data$lakename) #make sure you get the lake name right
...
NTL.phys.data.Tuesday.90s <- filter(NTL.phys.data, lakename=="Tuesday Lake", year4 < 2000, year4 > 1989)

```
Question: Why don't we filter using row numbers?

> Answer: You can't rely on the row numbers because they change depending on how the data is sorted so we don't control row numbers.
