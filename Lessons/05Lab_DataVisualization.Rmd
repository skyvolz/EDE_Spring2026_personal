...
PeterPaul.chem.nutrients %>% #change how month will show without changing df
...
PeterPaul.chem.nutrients %>% 
  filter(depth ==0) %>%  #filtering for surface temp
  ggplot(aes(
    x=daynum, y=temperature_C)) +
  geom_point()
...
PeterPaul.chem.nutrients %>% 
  filter(depth ==0) %>%  #filtering for surface temp
  ggplot(aes(
    x=daynum, y=temperature_C, color=year4)) +
  geom_point() +
  facet_grid(cols = vars(lakename))
...
PeterPaul.chem.nutrients %>% 
  ggplot(aes(
    x=temperature_C, y=sampledate, color=depth
  )) +
  geom_point()
...
PeterPaul.chem.nutrients %>% 
  ...
  geom_point(size=0.5) #size doesn't care about data - not put in aes()
...
#what is displayed in the text is a scale, which is a date
...
EPAair %>% 
  ggplot(aes(x=Ozone, y=PM2.5, color=mean_Lat)) +
  geom_point()
