...
> Answer: the system attaches the package's functions and datasets to your current session, like uploading an app to a phone
...
> ANSWER: USGS website using the current conditions tool, if someone were to try this again they would need to use the same time frame to get the same current conditions
...
> Answer: N/A values can cause values in summary stats. Calculating the mean of the original dataset makes the output be N/A, so any research question looking at the mean would require omitting data.
...
#code for date will only work if it matches the date string in the data - include slashes if it's in the data
```
