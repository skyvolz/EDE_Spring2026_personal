...
Terms of service say do not scrape the data. Some websites have a file called robots.txt is a guard agaisnt scraping.
Look to see if the site allows you to download it directly, or look for an API which allow you to scrape it.
Webpages that don't have API are just built for viewing, then the website might get overloaded during scraping and block you. To fix this you can add in pauses.
...
the_facility_name <- webpage %>% 
  html_nodes("tr:nth-child(2) th+ .left") %>% 
  html_text()
the_facility_name
